 ...
 *      Author: xz
 */
#ifndef ARACHNE_MESSAGE_H
#define ARACHNE_MESSAGE_H
...
typedef struct {
	...
	struct Arachne_payload {
		...
		float Sensor_Tv_Angle; //电视传感器视场角
		...
		float TurretAngle_Hor; //转塔方位角度
		float TurretAngle_Ver; //转塔俯仰角度
		struct Time_Date {
		...
		float Sensor_Thermal_Angle; //红外传感器视场角
	} payload;
} IPC_msg; //接收的数据包
